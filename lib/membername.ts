...
interface Stored { email: string; displayName: string; by?: string; at?: string }
...
function parse(message: string | null | undefined): Stored | null {
  ...
  try {
    ...
    return { email: o.email.toLowerCase(), displayName: typeof o.displayName === 'string' ? o.displayName : '' }
  } catch {
    return null
  }
}
...
export async function setMemberDisplayName(email: string, displayName: string, by: string): Promise<string | null> {
  ...
  const { error } = await supabase.from('chat_logs').insert([{
    ...
    message: JSON.stringify({ email: email.trim().toLowerCase(), displayName: clean, by, at: new Date().toISOString() }),
  }])
