...
export default function MembersPage() {
  ...
  if (!authChecked) {
    return <div className="min-h-screen bg-gray-100 flex items-center justify-center text-gray-500 text-sm">Checking access…</div>
  }
    ...
    <div className="min-h-screen bg-gray-100 text-gray-900">
      ...
      <div className="border-b border-gray-200 bg-white/95 backdrop-blur px-5 py-3 flex items-center justify-between sticky top-0 z-10">
        ...
        <button onClick={() => { setShowAdd(true); setAddForm(emptyForm); setError('') }}
          className="px-3.5 py-1.5 text-xs font-medium text-gray-900 rounded-lg transition-colors"
                    ...
                    <tr key={m.id} className="border-b border-gray-200 bg-gray-50 align-top">
                      ...
                      <td className="px-4 py-3">
                            ...
                            {sites.map((s) => {
                              ...
                              return (
                                <button key={s.site_id} type="button" onClick={() => setEditForm({ ...editForm, sites: toggleSite(editForm.sites, s.site_id) })}
                                  className={`text-[11px] px-2 py-1 rounded-full border transition-colors ${on ? 'bg-blue-600 border-blue-500 text-white' : 'bg-gray-200 border-gray-300 text-gray-500 hover:border-gray-400'}`}>
                                  {s.name}
                                </button>
                              )
                            })}
                        ...
                        <input type="password" value={editForm.password} onChange={(e) => setEditForm({ ...editForm, password: e.target.value })}
                          ...
                          className="mt-2 block w-56 bg-gray-200 border border-gray-300 rounded px-2 py-1 text-xs text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500" />
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1">
                          ...
                          <button onClick={() => setEditId(null)} className="text-xs bg-gray-200 hover:bg-gray-300 text-gray-700 px-2 py-1 rounded">Cancel</button>
                        </div>
                      </td>
                    </tr>
                  )
                        ...
                        {m.role === 'admin' ? (
                          ...
                          <div className="flex flex-wrap gap-1">
                            {m.assigned_sites.map((id) => (
                              <span key={id} className="text-[11px] px-2 py-0.5 rounded-full bg-gray-200 border border-gray-300 text-gray-700">{siteName(id)}</span>
                            ))}
                          </div>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {confirmDeleteId === m.id ? (
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => removeMember(m.id)} disabled={busyId === m.id} className="text-xs text-red-600 hover:text-red-700 font-semibold">Yes</button>
                            <span className="text-xs text-gray-500">·</span>
                            <button onClick={() => setConfirmDeleteId(null)} className="text-xs text-gray-500 hover:text-gray-600">No</button>
                          </div>
                        ) : (
      ...
      {/* Add member modal */}
      {showAdd && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-20" onClick={() => setShowAdd(false)}>
          <div className="bg-gray-100 border border-gray-200 rounded-2xl p-6 w-full max-w-md shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={addMember} className="space-y-4">
              <div>
                ...
                <input type="email" required value={addForm.email} onChange={(e) => setAddForm({ ...addForm, email: e.target.value })}
                  className="w-full bg-gray-200 border border-gray-300 rounded-lg px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500" placeholder="member@example.com" />
              </div>
              <div>
                ...
                <input type="text" required minLength={6} value={addForm.password} onChange={(e) => setAddForm({ ...addForm, password: e.target.value })}
                  className="w-full bg-gray-200 border border-gray-300 rounded-lg px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500" placeholder="At least 6 characters" />
              </div>
              <div>
                ...
                <select value={addForm.role} onChange={(e) => setAddForm({ ...addForm, role: e.target.value as 'admin' | 'standard' })}
                  className="w-full bg-gray-200 border border-gray-300 rounded-lg px-3 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-blue-500">
              ...
              {addForm.role === 'standard' && (
                <div>
                  <label className="block text-xs text-gray-500 font-medium mb-1.5">Assigned sites</label>
                    ...
                    {sites.map((s) => {
                      ...
                      return (
                        <button key={s.site_id} type="button" onClick={() => setAddForm({ ...addForm, sites: toggleSite(addForm.sites, s.site_id) })}
                          className={`text-xs px-2.5 py-1.5 rounded-full border transition-colors ${on ? 'bg-blue-600 border-blue-500 text-white' : 'bg-gray-200 border-gray-300 text-gray-500 hover:border-gray-400'}`}>
                          {s.name}
                        </button>
                      )
                    })}
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button type="button" onClick={() => setShowAdd(false)} className="px-4 py-2.5 bg-gray-200 hover:bg-gray-200 text-gray-700 rounded-lg text-sm">Cancel</button>
              </div>
            </form>
          </div>
        </div>
